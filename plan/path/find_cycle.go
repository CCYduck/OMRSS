...
func (path *Path) FindCyCle() (bool, []int) {
	var cyclelist []int
	for _, node := range path.Nodes {
		visited := make(map[int]bool)
		if DFSCyCle(path, node, visited, -1, node.ID) {
...
func DFSCyCle(path *Path, node *Node, visited map[int]bool, parentID int, startID int) bool {
	...
	for _, conn := range node.Connections {
		...
		toNode := path.GetNodeByID(conn.ToNodeID)
		if DFSCyCle(path, toNode, visited, node.ID, startID) {
			return true
		}
	}
	return false
...
func (path *Path) GetFeedbackEdgeSet(cyclelist []int, E []int) [][2]int {
	var E_prime [][2]int
	for _, cycle := range cyclelist {
		node := path.GetNodeByID(cycle)
		for _, conn := range node.Connections {
