...
func (osro *OSRO) OSRO_Run(network *network.Network, timeout_index int, method_id int, method_name string) Result {
	var result Result
	...
	// 3. 記錄最終結果
	resultObj, resultCost := schedule.OBJ(network, osro.KPath, osro.InputPath, osro.BGPath, method_name)
